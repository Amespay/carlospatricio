			...
			<ul class="header-address">
				<?php if ( RDTheme::$options['phone'] ): ?>
					<li><i class="fa fa-phone" aria-hidden="true"></i><a href="tel:<?php echo esc_attr( RDTheme::$options['phone'] );?>"><?php echo esc_html( RDTheme::$options['phone'] );?></a><span><?php $header_hotline_txt = RDTheme::$options['header_hotline_txt'];
						if ( !empty( $header_hotline_txt ) ){ echo esc_html( $header_hotline_txt ); } else { esc_html_e( 'Hotline', 'miako' ); } ?></span></li>
				<?php endif; ?>
				<?php if ( RDTheme::$options['address'] ): ?>
					<li><i class="fa fa-map-marker" aria-hidden="true"></i><?php echo wp_kses_post( RDTheme::$options['address'] );?><span><?php $header_location_txt = RDTheme::$options['header_location_txt']; if ( !empty( $header_location_txt ) ){ echo esc_html( $header_location_txt ); } else { esc_html_e( 'Location', 'miako' ); } ?></span></li>
				<?php endif; ?>
				<?php if ( RDTheme::$options['hour'] ): ?>
					<li><i class="fa fa-clock-o" aria-hidden="true"></i><?php echo esc_html( RDTheme::$options['hour'] );?><span><?php $header_officehour_txt = RDTheme::$options['header_officehour_txt']; if ( !empty( $header_officehour_txt ) ){ echo esc_html( $header_officehour_txt ); } else { esc_html_e( 'Office Hour', 'miako' ); } ?></span></li>
				<?php endif; ?>
			</ul>
		</div>
	</div>
