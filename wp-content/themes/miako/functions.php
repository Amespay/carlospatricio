...
// Widgets fallback
if ( function_exists( 'miako_core_load_textdomain' ) && !defined( 'MIAKO_CORE_UPDATE_1' ) ) {
	add_action( 'admin_notices', 'miako_widgets_fallback_notice' );
}

function miako_widgets_fallback_notice() {
	$notice = '<div class="error"><p>' . sprintf( __( "Please update plugin <b><i>Miako Core</b></i> to the latest version otherwise some functionalities will not work properly. You can update it from <a href='%s'>here</a>", 'miako' ), menu_page_url( 'miako-install-plugins', false ) ) . '</p></div>';
